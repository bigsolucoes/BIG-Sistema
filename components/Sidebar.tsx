...
const Sidebar: React.FC = () => {
  return (
    <div className={`w-64 p-5 flex flex-col space-y-6 h-full shadow-lg 
      ...
      <div className={`text-2xl font-bold text-center text-${ACCENT_COLOR}`}>
        {APP_NAME}
      </div>
          ...
          {NAVIGATION_ITEMS.map((item) => (
            <li key={item.name} className="mb-2">
              <NavLink
                to={item.path}
                className={({ isActive }) =>
                  `flex items-center space-x-3 p-3 rounded-lg hover:bg-slate-700 transition-colors duration-200 ease-in-out ${
      ...
      <div className="text-center text-xs text-slate-400">
        ...
        <p>BIG Soluções</p> {/* Updated branding */}
      </div>
    </div>
  );
};
